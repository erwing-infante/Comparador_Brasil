...
def send_result(a,hit,new_at=None):
    ...
    if hit:
        d=pct_drop(base,new_at);e=time.time()-a['alert_ts'];tg(f'✅ <b>RESULTADO ANTICIPADOR</b>\n\n📉 {lab}\nApuesta Total PA: {base:.3f} → <b>{new_at:.3f}</b>\nCaída: <b>-{d:.2f}%</b>\n⏱ Tardó: <b>{e:.1f} s</b>')
...
def main():
    ...
    mn={bm:os.path.getmtime(p) for bm,p in npaths.items()};mp={bm:os.path.getmtime(p) for bm,p in ppaths.items() if bm in {'apuestatotal','1xbet'}}
    ...
    while True:
            ...
            for event,nr in nopa[bm].items():
                ...
                for sel,(_,col) in SEL.items():
                    ...
                    if not rec or t-rec['first_ts']>MAX_CONFIRM_SEC:
                        pending[pkey]={'event':list(event),'sel':sel,'first_ts':t,'moves':{bm:move},'at_pa_base':quote(pa,'apuestatotal',event,sel),'x1_pa_base':quote(pa,'1xbet',event,sel)};continue
                    ...
                    append_result({'type':'signal','time':now().isoformat(),**alerts[aid]});cooldown[pkey]=t+COOLDOWN_SEC;pending.pop(pkey,None);save_state({'cooldown':cooldown})
        done=[]
        for aid,a in alerts.items():
            ...
            if cur is not None and cur<a['at_pa'] and pct_drop(a['at_pa'],cur)>=RESULT_MIN_DROP_PCT:
                append_result({'type':'result','result':'HIT','time':now().isoformat(),'alert_id':aid,'event':a['event'],'selection':a['sel'],'at_old':a['at_pa'],'at_new':cur,'drop_pct':pct_drop(a['at_pa'],cur),'elapsed_sec':t-a['alert_ts']});send_result(a,True,cur);done.append(aid);continue
            if t>=a['expires']:
                append_result({'type':'result','result':'MISS','time':now().isoformat(),'alert_id':aid,'event':a['event'],'selection':a['sel'],'at_old':a['at_pa'],'at_new':cur,'elapsed_sec':RESULT_WINDOW_SEC});send_result(a,False);done.append(aid)
