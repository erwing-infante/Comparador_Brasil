...
OUT_FILE = os.path.join(DATA_DIR, "cuotas_NoPA.json")

# ✅ NUEVO: directorio de histórico
HISTORICO_DIR = os.path.join(DATA_DIR, "historico_cuotas_NoPA")
...
def load_rows_from_file(path: str):
    ...
    for it in raw:
        ...
        # Fusionador NoPA:
        # Lee exclusivamente las columnas NoPA.
        local_odd = to_float(it.get("Cuota Local NoPA"))
        empate_odd = to_float(it.get("Cuota Empate"))
        visita_odd = to_float(it.get("Cuota Visita NoPA"))
...
def guardar_historico_cuotas(filas: list[dict], fuentes_ok: list[str], fuentes_error: list[str]) -> None:
    ...
    print(f"✔ Histórico NoPA actualizado: {path_historico}")

def fusionar_cuotas():
    print("Fusionando cuotas NoPA con equivalencias externas + similitud...")
